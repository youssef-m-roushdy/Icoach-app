...
export function useStepCounter(): StepData & {
  ...
  // ✅ FIX: Refs to track watchStepCount baseline and cumulative value
  const watchCumulativeRef = useRef<number>(0);
  const watchIsFirstReadingRef = useRef<boolean>(true);
  ...
  const startStepPersistence = useCallback((currentGoal: number) => {
    ...
    persistIntervalRef.current = setInterval(() => {
      if (isMountedRef.current && todayStepsRef.current > 0) {
        ...
        saveCachedData({ steps: todayStepsRef.current, goal: currentGoal });
      }
  ...
  const syncStepsWithBackend = useCallback(
    async (currentSteps: number, force: boolean = false, retryCount: number = 0): Promise<void> => {
      ...
      const now = Date.now();
      if (!force && now - lastSyncTimestampRef.current < SYNC_THROTTLE_MS) return;
  ...
  const updateGoal = useCallback(
    async (newGoal: number): Promise<void> => {
      ...
      try {
        ...
        const response = await dailyActiveService.updateGoal(validToken, { goal: newGoal });
  ...
  // ✅ FIX: Subscribe to pedometer with proper delta tracking
  const startPedometerSubscription = useCallback((syncTimeout: { current: ReturnType<typeof setTimeout> | null }, currentGoal: number) => {
    // Reset baseline tracking on every (re)subscription
    watchCumulativeRef.current = 0;
    watchIsFirstReadingRef.current = true;
    ...
    subscriptionRef.current = Pedometer.watchStepCount((res) => {
      ...
      // ✅ First reading establishes baseline — do NOT add steps
      if (watchIsFirstReadingRef.current) {
        watchCumulativeRef.current = res.steps;
        watchIsFirstReadingRef.current = false;
        console.log('👟📍 Pedometer baseline established:', res.steps);
        return;
      }

      // ✅ Compute real delta since last reading
      const delta = res.steps - watchCumulativeRef.current;
      watchCumulativeRef.current = res.steps;

      // ✅ Ignore zero/negative deltas (sensor noise, re-subscription artifacts)
      if (delta <= 0) return;

      todayStepsRef.current = todayStepsRef.current + delta;
      setSteps(todayStepsRef.current);

      console.log(`👟 +${delta} steps → total: ${todayStepsRef.current}`);

      const shouldSync =
        todayStepsRef.current % SYNC_STEP_INTERVAL === 0 ||
        (todayStepsRef.current >= currentGoal && lastSyncedStepsRef.current < currentGoal);
  ...
  // Main initialization
  useEffect(() => {
    const syncTimeout = { current: null as ReturnType<typeof setTimeout> | null };
    ...
    const init = async () => {
      ...
      try {
        ...
        // iOS: get accurate step count for today from HealthKit
        if (Platform.OS === 'ios') {
          ...
          // Android: start from cached steps
        ...
        startStepPersistence(goal);

        // ✅ Start pedometer subscription with correct delta tracking
        startPedometerSubscription(syncTimeout, goal);
    ...
    const appStateSub = AppState.addEventListener('change', async (state) => {
      if (state === 'active' && isMountedRef.current && initialLoadCompletedRef.current) {
        ...
        // ✅ Re-subscribe on resume to reset baseline (avoids +1 on reopen)
        startPedometerSubscription(syncTimeout, goal);
