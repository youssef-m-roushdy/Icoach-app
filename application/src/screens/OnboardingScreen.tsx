...
function useEntrance(
  ...
  useEffect(() => {
    if (!isActive) {
      anims.forEach((a) => { a.val.value = a.from; });
      return;
    }
    anims.forEach((a) => {
      a.val.value = a.from;
      const animation = a.useSpring
        ...
        : withTiming(a.to, { duration: a.duration ?? 500, easing: EASE_OUT });
...
const DotItem = memo(({ index, current }: { index: number; current: number }) => {
  ...
  useEffect(() => {
    ...
    width.value = withSpring(isActive ? 32 : 8, { damping: 18, stiffness: 200, mass: 0.6 });
...
const DatePickerWheel = memo(
  ({ items, selectedIndex, onSelect, width = 80 }: {
    items: string[]; selectedIndex: number; onSelect: (i: number) => void; width?: number;
  }) => {
            ...
            <View key={i} style={[styles.pickerItem, { height: IH }]}>
              <Text
                style={[
                  ...
                  i === selectedIndex && [styles.pickerTextSel, { color: colors.text }],
                ]}
              >
                {item}
              </Text>
...
const HeightRuler = memo(
  ({ value, onChange, min = 120, max = 220 }: {
    ...
    const ticks = useMemo(() =>
      Array.from({ length: max - min + 1 }, (_, i) => {
        const tv = max - i;
        return { tv, major: tv % 10 === 0, mid: tv % 5 === 0 && tv % 10 !== 0 };
      }), [min, max]);
...
const WeightGauge = memo(
  ({ value, onChange, height }: {
    ...
    useEffect(() => {
      setTimeout(() => {
        ref.current?.scrollTo({ x: (value - MIN) * TW - CENTER_OFFSET, animated: false });
      }, 50);
    }, []);

    const snap = useCallback((e: any) => {
      const v = Math.round((e.nativeEvent.contentOffset.x + CENTER_OFFSET) / TW) + MIN;
      const c = Math.max(MIN, Math.min(MAX, v));
      if (c !== lastValueRef.current) {
        lastValueRef.current = c;
        onChange(c);
    ...
    return (
      <View style={styles.weightWrap}>
        ...
        <View style={styles.weightDisplay}>
          <Text style={[styles.weightVal, { color: colors.text }]}>{value.toFixed(1)}</Text>
        ...
        <View style={[styles.bmiBox, { borderColor: bmiInfo.color }]}>
          <Text style={[styles.bmiBoxLbl, { color: colors.textSecondary }]}>Your BMI shows you are</Text>
          <Text style={[styles.bmiBoxVal, { color: bmiInfo.color }]}>{bmiInfo.label}</Text>
        </View>
      </View>
    );
  },
...
// ═════════════════════════════════════════════════════════════════════════════
// ═════════════════════════════════════════════════════════════════════════════
// STEP 1 — Gender (FIXED WITH ANIMATION)
// ═════════════════════════════════════════════════════════════════════════════
const Step1Gender = memo(
  ({ isActive, gender, setGender }: {
    isActive: boolean; gender: string; setGender: (g: 'male' | 'female') => void;
  }) => {
    const { colors } = useTheme();
    ...
    // Animated styles for scale only
    ...
    // Use direct boolean checks for colors (instant updates)
          ...
          <Animated.View style={maleWrapStyle}>
            ...
            >
              <Animated.View style={[
                ...
                maleScaleStyle,
                { 
                  borderColor: isMaleSelected ? MALE_COLOR : 'rgba(180,180,180,0.4)',
          ...
          <Animated.View style={femWrapStyle}>
            ...
            >
              <Animated.View style={[
                ...
                femScaleStyle,
                { 
                  borderColor: isFemaleSelected ? FEMALE_COLOR : 'rgba(180,180,180,0.4)',
                  backgroundColor: isFemaleSelected ? FEMALE_COLOR : 'transparent',
                }
              ]}>
                <Ionicons 
                  name="female" 
...
const Step2Birthday = memo(
  ({ isActive, days, months, years, birthDay, birthMonth, birthYear,
    currentYear, setBirthDay, setBirthMonth, setBirthYear }: any) => {
    const { colors } = useTheme();
    ...
    const titleStyle = useAnimatedStyle(() => ({
      opacity: titleO.value, transform: [{ translateY: titleY.value }],
    }));
    const subtStyle = useAnimatedStyle(() => ({
      opacity: subtO.value, transform: [{ translateY: subtY.value }],
    }));
    const cardStyle = useAnimatedStyle(() => ({
      opacity: cardO.value, transform: [{ scale: cardSc.value }],
    }));
    ...
    return (
      <View style={styles.stepInner}>
        ...
        <Animated.Text style={[styles.stepSubtitle, { color: colors.textSecondary }, subtStyle]}>
...
const Step3Height = memo(
  ({ isActive, height, gender, onChange }: {
    ...
    const imgStyle = useAnimatedStyle(() => ({
      opacity: imgO.value, transform: [{ translateX: imgX.value }],
    }));
    const rulStyle = useAnimatedStyle(() => ({
      opacity: rulO.value, transform: [{ translateX: rulX.value }],
    }));
    const valStyle = useAnimatedStyle(() => ({
      opacity: valO.value, transform: [{ scale: valSc.value }],
    }));
    ...
    return (
      <View style={[styles.stepInner, { flex: 1 }]}>
        ...
        <Animated.View style={[styles.heightValBox, { backgroundColor: colors.surface }, valStyle]}>
          <Text style={styles.heightValNum}>{height}</Text>
          <Text style={[styles.heightValUnit, { color: colors.textSecondary }]}>cm</Text>
        </Animated.View>
      </View>
    );
  },
...
const Step4Weight = memo(
  ({ isActive, weight, height, onChange }: {
    ...
    const gaugeStyle = useAnimatedStyle(() => ({
      opacity: gaugeO.value, transform: [{ scale: gaugeSc.value }],
    }));
...
// ═════════════════════════════════════════════════════════════════════════════
// STEP 5 — Goals (same appearance, smooth stagger entrance)
// ═════════════════════════════════════════════════════════════════════════════
const GOALS = [
...
const GoalItem = memo(({
  goal, isSelected, onPress, animO, animY, fillSV,
}: {
  goal: (typeof GOALS)[number]; isSelected: boolean; onPress: () => void;
  animO: SharedValue<number>; animY: SharedValue<number>; fillSV: SharedValue<number>;
}) => {
  const { colors } = useTheme();
  ...
  return (
    <Animated.View style={[styles.goalCardWrap, wrapStyle]}>
      <TouchableOpacity activeOpacity={0.85} onPress={onPress} style={{ borderRadius: 16, overflow: 'hidden' }}>
        <Animated.View style={[styles.goalCard, cardStyle]}>
          <Ionicons name={goal.icon as any} size={28} color={isSelected ? '#FFF' : C.primary} />
...
const ActivityItem = memo(({
  lvl, isSelected, onPress, animO, animY, fillSV,
}: {
  lvl: (typeof ACTIVITIES)[number]; isSelected: boolean; onPress: () => void;
  animO: SharedValue<number>; animY: SharedValue<number>; fillSV: SharedValue<number>;
}) => {
  const { colors } = useTheme();
  ...
  return (
    <Animated.View style={wrapStyle}>
      <TouchableOpacity activeOpacity={0.85} onPress={onPress} style={{ borderRadius: 12, overflow: 'hidden' }}>
        <Animated.View style={[styles.actCard, cardStyle]}>
          <Text style={[styles.actLabel, { color: isSelected ? '#FFF' : colors.text }]}>
            {lvl.label}
          </Text>
          <Text style={[styles.actDesc, { color: isSelected ? 'rgba(255,255,255,0.75)' : colors.textSecondary }]}>
            {lvl.desc}
          </Text>
        </Animated.View>
      </TouchableOpacity>
...
const Step5Goals = memo(
  ({ isActive, fitnessGoal, activityLevel, setFitnessGoal, setActivityLevel }: {
    isActive: boolean; fitnessGoal: string; activityLevel: string;
    setFitnessGoal: (v: any) => void; setActivityLevel: (v: any) => void;
  }) => {
    const { colors } = useTheme();

    // Title / subtitle / section label
    const titleO = useSharedValue(0);
    const titleY = useSharedValue(30);
    const subtO = useSharedValue(0);
    const subtY = useSharedValue(30);
    ...
    // Goal anim (3)
    const g0O = useSharedValue(0); const g0Y = useSharedValue(35);
    const g1O = useSharedValue(0); const g1Y = useSharedValue(35);
    const g2O = useSharedValue(0); const g2Y = useSharedValue(35);
    ...
    // Activity anim (4)
    const a0O = useSharedValue(0); const a0Y = useSharedValue(35);
    const a1O = useSharedValue(0); const a1Y = useSharedValue(35);
    const a2O = useSharedValue(0); const a2Y = useSharedValue(35);
    const a3O = useSharedValue(0); const a3Y = useSharedValue(35);
    ...
    useEffect(() => {
      if (!isActive) {
        titleO.value = 0; titleY.value = 30;
        subtO.value = 0; subtY.value = 30;
        lblO.value = 0; lblY.value = 30;
      ...
      // Goals stagger
      goalOs.forEach((o, i) => {
        o.value = withDelay(250 + i * 100, withTiming(1, { duration: 420, easing: EASE_OUT }));
      });
      goalYs.forEach((y, i) => {
        y.value = withDelay(250 + i * 100, withTiming(0, { duration: 420, easing: EASE_OUT }));
      });

      // Activity label
      const afterGoals = 250 + GOALS.length * 100 + 80;
      lblO.value = withDelay(afterGoals, withTiming(1, { duration: 380, easing: EASE_OUT }));
      lblY.value = withDelay(afterGoals, withTiming(0, { duration: 380, easing: EASE_OUT }));

      // Activities stagger
      actOs.forEach((o, i) => {
        o.value = withDelay(afterGoals + 60 + i * 80, withTiming(1, { duration: 380, easing: EASE_OUT }));
      });
      actYs.forEach((y, i) => {
        y.value = withDelay(afterGoals + 60 + i * 80, withTiming(0, { duration: 380, easing: EASE_OUT }));
      });
    }, [isActive]);
    ...
    useEffect(() => {
      actFills.forEach((f, i) => {
        f.value = withTiming(ACTIVITIES[i].id === activityLevel ? 1 : 0, FAST_TIMING);
      });
    }, [activityLevel]);
    ...
    const lblStyle = useAnimatedStyle(() => ({
      opacity: lblO.value, transform: [{ translateY: lblY.value }],
    }));
...
// ═════════════════════════════════════════════════════════════════════════════
const AnimatedSlide = memo(({ index, railX, children }: {
  index: number; railX: SharedValue<number>; children: React.ReactNode;
}) => {
  const contentStyle = useAnimatedStyle(() => {
    ...
    return {
      transform: [
        { translateX: interpolate(distance, [-1, 0, 1], [-W * 0.2, 0, W * 0.2], Extrapolation.CLAMP) },
        { scale: interpolate(Math.abs(distance), [0, 1], [1, 0.92], Extrapolation.CLAMP) },
      ],
      opacity: interpolate(Math.abs(distance), [0, 0.4, 1], [1, 0.7, 0.2], Extrapolation.CLAMP),
    };
  });
  ...
  return (
    <View style={styles.slide}>
      <Animated.View style={[{ flex: 1 }, contentStyle]}>
        <ScrollView contentContainerStyle={styles.slideScroll} showsVerticalScrollIndicator={false} nestedScrollEnabled>
          {children}
        </ScrollView>
      </Animated.View>
    </View>
...
export default function OnboardingScreen() {
  ...
  const years = useMemo(() => Array.from({ length: 80 }, (_, i) => String(currentYear - 80 + i)), []);
  ...
  const updateStep = useCallback((nextStep: number) => { setStep(nextStep); }, []);
  ...
  const panGesture = Gesture.Pan()
    ...
    .onUpdate((e) => {
      ...
      if ((stepSV.value === 0 && dx > 0) || (stepSV.value === TOTAL_STEPS - 1 && dx < 0)) {
        dx *= 0.15;
      }
      railX.value = base + dx;
    })
    .onEnd((e) => {
      ...
      let next = cur;
      if ((e.translationX < -SWIPE_THRESHOLD || e.velocityX < -SWIPE_VELOCITY) && cur < TOTAL_STEPS - 1) {
        next = cur + 1;
      } else if ((e.translationX > SWIPE_THRESHOLD || e.velocityX > SWIPE_VELOCITY) && cur > 0) {
        next = cur - 1;
      }
      goTo(next);
    });
    ...
    <View style={[styles.root, { backgroundColor: colors.background }]}>
      ...
      <View style={styles.header}>
        <Animated.View style={backStyle}>
          <TouchableOpacity onPress={handleBack} style={styles.backBtn} disabled={step === 0}>
            <Ionicons name="chevron-back" size={22} color={C.primary} />
            <Text style={styles.backTxt}>Back</Text>
          </TouchableOpacity>
        </Animated.View>
        <TouchableOpacity onPress={() => navigation.replace('Home')} style={styles.headerSide}>
          ...
          >
            {isLoading ? (
              ...
              <View style={styles.btnContent}>
                <Text style={styles.btnTxt}>{isLastStep ? 'Get Started' : 'Continue'}</Text>
                {!isLastStep && <Ionicons name="arrow-forward" size={20} color="#FFF" style={{ marginLeft: 8 }} />}
              </View>
            )}
          </TouchableOpacity>
        </Animated.View>
...
const styles = StyleSheet.create({
  ...
  header: {
    ...
    paddingHorizontal: 16, paddingTop: Platform.OS === 'ios' ? 60 : 44, paddingBottom: 4,
  },
  headerSide: { width: 70 },
  ...
  stepCounter: { alignItems: 'center', paddingBottom: 4 },
  stepCounterText: { fontSize: 13, fontWeight: '500' },

  railClip: { flex: 1, overflow: 'hidden' },
  rail: { flex: 1, flexDirection: 'row', width: W * TOTAL_STEPS },
  slide: { width: W, flex: 1 },
  slideScroll: { flexGrow: 1, paddingHorizontal: 24, paddingTop: 12, paddingBottom: 16 },
  stepInner: { flex: 1, paddingTop: 8 },

  stepTitle: { fontSize: 28, fontWeight: '800', textAlign: 'center', marginBottom: 8, letterSpacing: -0.5 },
  stepSubtitle: { fontSize: 16, textAlign: 'center', marginBottom: 36, lineHeight: 24, paddingHorizontal: 16 },

  // Gender — vertical
  genderColumn: { alignItems: 'center', marginTop: 36, gap: 25 },
...
genderLabel: { fontSize: 20, fontWeight: '600' },
  ...
  goalCardWrap: { flex: 1 },
  goalCard: { borderRadius: 16, paddingVertical: 20, alignItems: 'center', borderWidth: 1.5 },
  goalText: { fontSize: 13, fontWeight: '600', marginTop: 8 },
  sectionLabel: { fontSize: 16, fontWeight: '700', marginTop: 28, marginBottom: 14 },
  activityList: { gap: 10 },
  actCard: { borderRadius: 12, padding: 16, borderWidth: 1.5 },
  actLabel: { fontSize: 16, fontWeight: '600' },
  actDesc: { fontSize: 13, marginTop: 2 },

  // Button
  btnWrap: { paddingHorizontal: 24, paddingBottom: Platform.OS === 'ios' ? 40 : 24, paddingTop: 12 },
  btn: {
    borderRadius: 30, paddingVertical: 18, alignItems: 'center', justifyContent: 'center',
