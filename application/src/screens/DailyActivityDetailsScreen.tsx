// screens/DailyActivityDetailsScreen.tsx
...
export default function DailyActivityDetailsScreen({ navigation }: any) {
  ...
  const loadAllData = useCallback(async () => {
    ...
    try {
      const [statsRes, historyRes, streakRes, pointsRes] = await Promise.all([
        dailyActiveService.getStats(token),
        dailyActiveService.getHistory(token, { limit: 90 }),
        dailyActiveService.getStreak(token),
        dailyActiveService.getTotalPoints(token),
      ]);
      
      if (statsRes.success && statsRes.data) {
        setStats(statsRes.data);
        // Set longest streak from stats
        if (statsRes.data.longestStreak !== undefined) {
          setStreak(prev => ({ 
            ...prev, 
      ...
      if (streakRes && streakRes.currentStreak !== undefined) {
        setStreak(prev => ({ 
          ...prev, 
      ...
      if (pointsRes && pointsRes.totalPoints !== undefined) {
        setTotalPoints(pointsRes.totalPoints || 0);
      }
      
    } catch (error) {
      console.error('Failed to load daily activity data:', error);
    } finally {
      setLoading(false);
    }
  }, [token]);
  ...
  const handleUpdateGoal = async (newGoal: number) => {
    ...
    try {
      const response = await dailyActiveService.updateGoal(token!, { goal: newGoal });
      if (response.success) {
        setModalMessage(`Step goal updated to ${newGoal.toLocaleString()} steps successfully!`);
  ...
  const renderDayBar = (day: any, index: number, maxSteps: number) => {
    ...
    return (
      <View key={index} style={styles.dayBarContainer}>
        ...
        <Text style={[styles.dayAmount, { color: colors.textSecondary }]}>
          {dailyActiveService.formatSteps(day.steps || 0)}
        </Text>
      </View>
    );
  };
  ...
  const renderHistoryItem = (item: any) => {
    ...
    const goal = item.goal || 10000;
    const steps = item.stepCount || item.steps || 0;
      ...
      <View key={item.id} style={[styles.historyItem, { backgroundColor: colors.authInputBg || colors.surface, borderColor: colors.authInputBorder || colors.cardBorder }]}>
        ...
        <View style={styles.historyAmounts}>
          <View style={styles.historyAmount}>
            ...
            <Text style={[styles.historyAmountUnit, { color: colors.textSecondary }]}>steps</Text>
          </View>
        ...
        {(item.pointsEarned > 0) && (
          <View style={[styles.pointsBadge, { backgroundColor: '#F59E0B20' }]}>
            ...
            <Text style={[styles.pointsText, { color: '#F59E0B' }]}>+{item.pointsEarned} pts</Text>
          </View>
        )}
      </View>
    );
  ...
  if (loading) {
    return (
      <View style={[styles.loadingContainer, { backgroundColor: colors.background }]}>
        ...
        <Text style={[styles.loadingText, { color: colors.textSecondary }]}>Loading activity data...</Text>
      </View>
    );
  }

  const weeklyData = stats?.weeklyData || [];
  const maxSteps = Math.max(...weeklyData.map((d: any) => d.steps || 0), 10000);
  const todayStats = stats?.today;
  const currentGoal = todayStats?.goal || 10000;
  const todaySteps = todayStats?.stepCount || todayStats?.steps || 0;
  const isCompleted = todayStats?.isCompleted || false;
  const totalStepsValue = stats?.totalSteps || 0;
  const averageStepsValue = stats?.averageDailySteps || 0;
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background, paddingTop: insets.top }]}>
      ...
      <View style={[styles.header, { backgroundColor: colors.surface + '95' }]}>
        ...
        <Text style={[styles.headerTitle, { color: colors.text }]}>Step Activity Details</Text>
      ...
      >
          ...
          <View style={[styles.todayCard, { backgroundColor: colors.authInputBg || colors.surface, borderColor: colors.authInputBorder || colors.cardBorder }]}>
            <View style={styles.todayMain}>
              <View style={styles.todayAmount}>
                ...
                <Text style={[styles.todayAmountUnit, { color: colors.textSecondary }]}>steps</Text>
              </View>
              ...
              <View style={styles.todayGoal}>
                ...
                <Text style={[styles.todayGoalUnit, { color: colors.textSecondary }]}>steps</Text>
              </View>
            </View>
            ...
            <View style={[styles.todayProgress, { backgroundColor: colors.progressBg }]}>
              <View 
                style={[
                  ...
                  { 
                    width: `${Math.min((todaySteps / currentGoal) * 100, 100)}%`,
                    backgroundColor: isCompleted ? '#10B981' : colors.primary 
                  }
                ]} 
              />
        ...
        <View style={styles.section}>
          ...
          <View style={styles.statsGrid}>
            {renderStatCard('Current Streak', `${streak.currentStreak || 0}`, 'days', 'fire', '#F59E0B')}
            {renderStatCard('Longest Streak', `${streak.longestStreak || 0}`, 'days', 'trophy', '#10B981')}
            {renderStatCard('Total Points', `${(totalPoints || 0).toLocaleString()}`, 'points', 'star', '#8B5CF6')}
            {renderStatCard('Total Steps', `${(totalStepsValue).toLocaleString()}`, 'steps', 'shoe-print', colors.primary)}
          </View>
        </View>
        ...
        <View style={styles.section}>
          ...
          <View style={[styles.chartCard, { backgroundColor: colors.authInputBg || colors.surface, borderColor: colors.authInputBorder || colors.cardBorder }]}>
            <View style={styles.barChart}>
              {weeklyData.map((day: any, index: number) => renderDayBar(day, index, maxSteps))}
            </View>
          </View>
        </View>
        ...
        <View style={styles.section}>
          ...
          {history.length > 0 ? (
            ...
            <View style={[styles.emptyContainer, { backgroundColor: colors.authInputBg || colors.surface, borderColor: colors.authInputBorder || colors.cardBorder }]}>
              ...
              <Text style={[styles.emptyText, { color: colors.textSecondary }]}>No step activity history yet</Text>
              <Text style={[styles.emptySubtext, { color: colors.textSecondary }]}>Start walking to track your steps!</Text>
            </View>
          )}
        </View>
      </ScrollView>
...
const styles = StyleSheet.create({
  ...
  pointsBadge: { position: 'absolute', top: 14, right: 14, flexDirection: 'row', alignItems: 'center', gap: 4, paddingHorizontal: 8, paddingVertical: 4, borderRadius: 12 },
